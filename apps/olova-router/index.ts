 ...
 * Auto-generates route.tree.ts when folders change
 */
...
interface LayoutEntry {
  path: string;           // Route path prefix this layout applies to
  filePath: string;       // Absolute path to the _layout.tsx file
}
...
function pathToRoute(relativePath: string) {
  const params: string[] = [];
  let hasCatchAll = false;
  const segments = relativePath.split(path.sep).filter(Boolean);
  
  const routeSegments = segments
    .filter(segment => !isRouteGroup(segment))  // Exclude route groups from URL
    .map(segment => {
...
/**
 * Detect export type from file content
 * Returns: { hasDefault: boolean, namedExport: string | null }
 */
function detectExportType(filePath: string): { hasDefault: boolean; namedExport: string | null } {
  try {
    ...
    // Check for named export: export const/function/class Name
    ...
    return { hasDefault: true, namedExport: null }; // Default assumption
  }
}
...
function scanDirectory(
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      if (entry.name === 'olova-router' || entry.name === 'assets' || entry.name.startsWith('_')) continue;
      ...
      if (baseName === '_layout' && extensions.includes(ext)) {
        ...
        const { routePath } = pathToRoute(relativePath);
        layouts.push({ 
      ...
      // Detect 404.tsx files
        ...
        // Calculate path prefix, excluding route groups
        ...
        const { routePath, params } = pathToRoute(relativePath);
...
function scanRoutes(rootDir: string, extensions: string[]): ScanResult {
  ...
  // Sort notFoundPages by pathPrefix length descending (most specific first)
  notFoundPages.sort((a, b) => b.pathPrefix.length - a.pathPrefix.length);
  // Sort layouts by path length descending (most specific first)
...
export function olovaRouter(options: OlovaRouterOptions = {}): Plugin {
  ...
  function startWatcher() {
    ...
    watcher = fs.watch(absoluteRootDir, { recursive: true }, (_eventType, filename) => {
      ...
      if (isIndexFile || isAppFile || is404File || isLayoutFile || isDirectory) {
