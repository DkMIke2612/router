...
function generateRouteTree(
  ...
  const routeObjects = routes.map((route, index) => {
    ...
    let loaderBody = `import('${relativePath}')`;
    if (!route.hasDefault && route.namedExport) {
      loaderBody += `.then(m => ({ ...m, default: m.${route.namedExport} }))`;
    }

    return `  { path: '${route.path}', loader: () => ${loaderBody} }`;
  }).join(',\n');
  ...
  const allImports = [notFoundImports, layoutImports].filter(Boolean).join('\n');
